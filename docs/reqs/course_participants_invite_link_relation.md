...
- [x] Создать миграцию для добавления полей `invite_link_id` и `course_group_id`
- [x] Заполнить `course_group_id` для существующих записей (где возможно) - автоматически в миграции
- [x] Добавить внешние ключи
- [x] Создать индексы
- [x] Обновить логику создания участника через invite link
- [x] Обновить API endpoints для работы с участниками
- [x] Обновить фронтенд для отображения информации о invite link
- [x] Обновить документацию
- [x] Добавить копирование `course_code` из таблицы `course` при создании участника
- [ ] Обновить модель SQLAlchemy `CourseParticipant` (если используется в backend)
- [ ] Добавить API для статистики по invite links (опционально)
...
**Статус:** Реализовано  
**Реализовано:**
- ✅ Миграция 0006 создана и готова к применению
- ✅ Добавлены поля `invite_link_id` и `course_group_id` в таблицу `courseparticipants`
- ✅ Созданы foreign key constraints и индексы
- ✅ API endpoints обновлены для работы с новыми полями
- ✅ При создании участника автоматически копируется `course_code` из таблицы `course`
- ✅ Документация по БД обновлена

**Следующие шаги:**
1. Применить миграцию 0006 к базе данных
2. Тестирование всех сценариев использования
3. Обновление существующих записей с NULL course_group_id (где возможно)
