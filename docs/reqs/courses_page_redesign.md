...
- Отображает список ботов, на которых курс развернут через группы (через таблицу `group`)
- Если курс не развернут ни на одном боте (нет активных групп), отображается текст: "No bots connected". 
- Если курс развернут на ботах, отображается список имен ботов (через `bot.bot_name`)
- Формат отображения: badges/tags - ссылки на бота в /bots
...
1. **Проверка зависимостей:**
   - Система проверяет, есть ли у курса активные группы (записи в `group` для данного `course_id`)
   - Если есть группы (курс подключен к ботам через группы), удаление запрещается
...
3. **Если курс подключен к ботам:**
   ...
   - Текст сообщения: "Cannot delete course. This course is connected to one or more groups. Please remove all groups before deleting the course."
...
- `GET /api/v1/courses`
- Возвращает список курсов с информацией о группах
- Фильтрация по `account_id = 1` (упрощено)
- Включает информацию о ботах, на которых развернут курс через группы

**Endpoint для удаления курса:**
- `DELETE /api/v1/courses/[courseId]`
- Проверяет наличие групп перед удалением
- Возвращает ошибку, если курс подключен к ботам через группы
...
- Обновление API endpoint для получения списка курсов (добавить информацию о группах)
- Создание API endpoint для копирования курса
- Обновление логики удаления курса (проверка наличия групп)
...
1. Пользователь кликает на кнопку "Delete" для курса с группами
2. Отображается сообщение об ошибке
3. Курс не удаляется
4. Пользователь видит информацию о том, что нужно сначала удалить группы
