...
| `course_code` | text | Идентификатор курса (legacy, для обратной совместимости). Копируется из таблицы `course` при создании участника. Если `course_code` равен `null` в таблице `course`, используется пустая строка `''`. |
