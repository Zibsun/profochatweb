...
-- Date: 2024
-- 
-- This script performs a phased migration:
-- Phase 0: Add account_id columns (backward compatible)
-- Phase 1: Create new tables (account, account_member, bot)
-- Phase 2: Migrate data and create relationships
-- Phase 3: Add new columns and foreign keys
-- Phase 4: Create indexes and constraints
--
-- IMPORTANT: 
-- - Run this script in a transaction
...
SELECT DISTINCT
    ...
    '' as bot_token,  -- Will need to be filled manually or from env
...
-- Add foreign keys for course_element
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
...
CREATE INDEX IF NOT EXISTS idx_course_created ON public.course (account_id, created_at DESC);
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'run_one_active_per_bot_chat'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'courseparticipants_unique'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'bannedparticipants_unique'
    ) THEN
